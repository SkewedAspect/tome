    ...
    return Promise.join(
        ...
        .then(() =>
        {
            return Promise.join(
                ...
                knex('page').insert({ path: '/perm', title: 'Perm Wiki Page', action_view: 'special', action_modify: 'special' })
                    .then(([ page_id ]) => knex('revision').insert({ page_id, body: "This is a _permissioned_ wiki page." })),
                knex('page').insert({ path: '/normal', title: 'Normal Wiki Page' })
                    .then(([ page_id ]) =>
                    {
                        return knex('revision').insert({ page_id, body: "This is a _normal_ wiki page. First Revision." })
                            .delay(20)
                            .then(() => knex('revision').insert({ page_id, body: "This is a _normal_ wiki page. Second Revision" }))
                            .delay(20)
                            .then(() => knex('revision').insert({ page_id, body: "This is a _normal_ wiki page." }));
                    }),
